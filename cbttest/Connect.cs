...
namespace cbttest
{
    class Connect
    {
        ...
        //Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\Users\mujahid41\Desktop\mahfuz\mahfuz_result\mahfuz_result\bin\Debug\DATA\RESULT_DB.accdb
        ...
        {
            OleDbDataAdapter da = new OleDbDataAdapter(strQry, con);
            DataSet ds = new DataSet();
            da.Fill(ds);
            int len = ds.Tables[0].Rows.Count;
            if (len > 0)
        ...
        public OleDbDataReader readdata(string str)
        {
            if (con.State != ConnectionState.Open)
            {
                con.Open();
